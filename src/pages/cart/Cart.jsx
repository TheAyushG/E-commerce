...
*/
  ...
  const buyNow = async () => {
    ...
    //payment integration
    var options = {
    key: "rzp_test_TkozZZ0nrmu8BB",
    key_secret: "iUdSqbr0WSY2v8SMIEBuxry2",
    ...
    handler: function (response) {
        ...
        // store in firebase  store it in firebase
        const orderInfo = {
          cartItems,
          addressInfo,
          date: new Date().toLocaleString(
        ...
        try {
          const result = addDoc(collection(fireDB, "orders"), orderInfo)    //addDoc take two data
        } catch (error) {
          console.log(error)
        }
    },
...
{/* imageUrl */}  <img src={imageUrl} alt="product-image" className="w-full rounded-lg sm:w-40" />
